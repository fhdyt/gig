...
if (!function_exists('jenis_barang')) {
  function jenis_barang()
  {
    $jenis_barang = array(
      'Pipa' => 'Pipa',
      'Fitting' => 'Fitting',
      'Asesoris Gas Medis' => 'Asesoris Gas Medis',
      'Tabung' => 'Tabung',
      'Cylinder Equipment' => 'Cylinder Equipment',
    );
    return $jenis_barang;
  }
}
...
if (!function_exists('relasi_list')) {
  function relasi_list()
  {
    $CI = &get_instance();
    $CI->load->database();
    $hasil = $CI->db->query('SELECT * FROM MASTER_RELASI WHERE RECORD_STATUS="AKTIF"')->result();
    return $hasil;
  }
}
